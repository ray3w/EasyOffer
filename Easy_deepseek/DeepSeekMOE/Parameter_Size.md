...
基于一组更加简单的配置参数：`dim=16, n_experts=32, n_groups=8, topk_groups=2, topk=2`，展示具体的示例，详细追踪每一步的计算过程和具体数值变化。详细追踪批次输入张量`[2, 4, 16]`（2个序列，每个序列4个token，每个token 16维特征）通过MoE模型的完整计算过程。
...
这个例子清晰展示了MoE的高效性：
- 总共32个专家，但每个token只调用2个专家(~6.25%的计算量)
- 专家计算负载分布：共16个token计算(8个token各2个专家)分散到10个活跃专家上
- 最活跃的专家11仅处理了3个token(~38%的总token)
- 22个专家(~69%)完全未被使用，节省了大量计算资源

这种稀疏激活机制是MoE能够大幅增加模型参数量而不显著增加计算量的核心原因。
