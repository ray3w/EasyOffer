    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        返回:
            Tuple[torch.Tensor, torch.Tensor]: 
                - 路由权重：每个选定专家的权重 [batch_size*seq_len, topk]
                - 专家索引：选择的专家ID [batch_size*seq_len, topk]
        """
        # 计算每个输入对每个专家的分数
        ...
        # 处理分组路由
        if self.n_groups > 1:
            # 将分数重塑为 [batch_size*seq_len, n_groups, experts_per_group]
...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        返回:
            torch.Tensor: 经专家处理后的输出 [batch_size, seq_len, dim]
        """
        # 保存原始形状并重塑为2D张量以简化处理
        shape = x.size()
        x = x.view(-1, self.dim)
